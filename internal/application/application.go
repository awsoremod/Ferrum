...
type Application struct {
	...
	secretKey      []byte
	serverData     *in_memory.ServerData
 ...
 */
func NewAppWithConfigs(configFile string) *Application {
	app := &Application{}
 ...
 * Returns: new Application as AppRunner
 */
func NewAppWithData(appConfig *config.AppConfig, serverData *in_memory.ServerData, secretKey []byte) *Application {
...
func (app *Application) initDataProviders() error {
	...
	if app.appConfig.DataSource.Type == config.FILE && app.dataConfigFile == nil {
		repo := in_memory.NewInMemoryRepo("", nil) // TODO sia
		repo.UsingData(*app.serverData)
		app.dataProvider = repo
		return nil
	}
...
func (app *Application) initRestApi() error {
	...
	securityService := services.NewSecurityService(app.dataProvider, app.logger)
	...
	app.webApiContext = &rest.WebApiContext{
		...
		TokenGenerator: &services.JwtGeneratorService{SignKey: app.secretKey, Logger: app.logger}, Logger: app.logger,
	}
	...
	app.httpHandler = app.newHttpLoggingHandler(appenderIndex, router)
	return nil
}
...
func (app *Application) newHttpLoggingHandler(index int, router *mux.Router) http.Handler {
	var resultRouter http.Handler = router
