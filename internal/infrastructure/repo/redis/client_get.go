 ...
 */
func (mn *RedisRepo) GetClientsFromRealm(realmName string) ([]domain.Client, error) {
	realmClients, err := mn.getRealmClients(realmName)
	if err != nil {
		return nil, err
	}

	clients := make([]domain.Client, len(realmClients))
 ...
 */
func (mn *RedisRepo) GetClient(clientName string) (*domain.Client, error) {
	clientKey := sf.Format(clientKeyTemplate, mn.namespace, clientName)
	client := getObjectFromRedis[domain.Client](mn.redisClient, mn.ctx, mn.logger, Client, clientKey)
	if client == nil {
...
func (mn *RedisRepo) GetClientFromRealm(realmName string, clientName string) (*domain.Client, error) {
...
func (mn *RedisRepo) getRealmClient(realmName string, clientName string) (*domain.ExtendedIdentifier, error) {
	realmClients, err := mn.getRealmClients(realmName)
	if err != nil {
		return nil, err
	}
	...
	var client domain.ExtendedIdentifier
...
func (mn *RedisRepo) getRealmClients(realmName string) ([]domain.ExtendedIdentifier, error) {
	realmClientsKey := sf.Format(realmClientsKeyTemplate, mn.namespace, realmName)
	realmClients := getObjectsListFromRedis[domain.ExtendedIdentifier](mn.redisClient, mn.ctx, mn.logger, RealmClients, realmClientsKey)
