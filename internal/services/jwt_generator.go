 ...
 */
func (generator *JwtGeneratorService) GenerateJwtAccessToken(realmBaseUrl string, tokenType string, scope string, sessionData *domain.UserSession,
 ...
 */
func (generator *JwtGeneratorService) GenerateJwtRefreshToken(realmBaseUrl string, tokenType string, scope string, sessionData *domain.UserSession) string {
...
func (generator *JwtGeneratorService) generateJwtAccessToken(tokenData *domain.AccessTokenData) string {
...
func (generator *JwtGeneratorService) generateJwtRefreshToken(tokenData *domain.TokenRefreshData) string {
...
func (generator *JwtGeneratorService) prepareAccessToken(realmBaseUrl string, tokenType string, scope string, sessionData *domain.UserSession,
	userData *domain.User,
) *domain.AccessTokenData {
	issuer := realmBaseUrl
	jwtCommon := domain.JwtCommonInfo{
	...
	accessToken := domain.NewAccessToken(&jwtCommon, userData)
	return accessToken
}

// prepareRefreshToken builds data.TokenRefreshData from a lot of params
func (generator *JwtGeneratorService) prepareRefreshToken(realmBaseUrl string, tokenType string, scope string, sessionData *domain.UserSession) *domain.TokenRefreshData {
	issuer := realmBaseUrl
	jwtCommon := domain.JwtCommonInfo{
	...
	accessToken := domain.NewRefreshToken(&jwtCommon)
	return accessToken
}

// makeSignedToken this function adds signature to token
func (generator *JwtGeneratorService) makeSignedToken(token *jwt.Token, tokenData *domain.AccessTokenData, signKey interface{}) (string, error) {
	var err error
	var sig string
	var jsonValue []byte
