...
import (
	...
	"github.com/wissance/Ferrum/internal/transport/rest/errors_rest"
)

type RepoForTokenBasedSecurityService interface {
	GetUserFromRealm(realmName string, userName string) (*domain.User, error)
...
type TokenBasedSecurityService struct {
	repo         RepoForTokenBasedSecurityService
 ...
 */
func NewSecurityService(repo RepoForTokenBasedSecurityService, logger *logging.AppLogger) *TokenBasedSecurityService {
	pwdSecService := &TokenBasedSecurityService{repo: repo, UserSessions: map[string][]domain.UserSession{}, logger: logger}
 ...
 */
func (service *TokenBasedSecurityService) Validate(tokenIssueData *dto.TokenGenerationData, realm *domain.Realm) *domain.OperationError {
	for _, c := range realm.Clients {
		if c.Name == tokenIssueData.ClientId {
			if c.Type == domain.Public {
				service.logger.Trace("Public client was successfully validated")
				return nil
			}

			// here we make deal with confidential client
			if c.Auth.Type == domain.ClientIdAndSecrets && c.Auth.Value == tokenIssueData.ClientSecret {
	...
	return &domain.OperationError{Msg: errors_rest.InvalidClientMsg, Description: errors_rest.InvalidClientCredentialDesc}
}
 ...
 */
func (service *TokenBasedSecurityService) CheckCredentials(tokenIssueData *dto.TokenGenerationData, realm *domain.Realm) *domain.OperationError {
	user, err := service.repo.GetUserFromRealm(realm.Name, tokenIssueData.Username)
	if err != nil {
		service.logger.Trace("Credential check: username mismatch")
		return &domain.OperationError{Msg: errors_rest.InvalidUserCredentialsMsg, Description: errors_rest.InvalidUserCredentialsDesc}
	}
 ...
 */
func (service *TokenBasedSecurityService) GetCurrentUserByName(realm *domain.Realm, userName string) *domain.User {
	user, _ := service.repo.GetUserFromRealm(realm.Name, userName) // TODO(sia) err ignored
	return user
}
 ...
 * Returns user from DataProvider or nil (user not found)
 */
func (service *TokenBasedSecurityService) GetCurrentUserById(realm *domain.Realm, userId uuid.UUID) *domain.User {
	user, _ := service.repo.GetUserFromRealmById(realm.Name, userId)
	return user
}
 ...
 */
func (service *TokenBasedSecurityService) GetSession(realm string, userId uuid.UUID) *domain.UserSession {
	realmSessions, ok := service.UserSessions[realm]
	if !ok {
		return nil
	}
 ...
 * Returns data.UserSession if found or nil
 */
func (service *TokenBasedSecurityService) GetSessionByAccessToken(realm string, token *string) *domain.UserSession {
	realmSessions, ok := service.UserSessions[realm]
	if !ok {
		return nil
	}
 ...
 * Returns data.UserSession if found or nil
 */
func (service *TokenBasedSecurityService) GetSessionByRefreshToken(realm string, token *string) *domain.UserSession {
	realmSessions, ok := service.UserSessions[realm]
	if !ok {
		return nil
	}
