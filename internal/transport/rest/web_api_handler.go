...
import (
	...
	"github.com/wissance/Ferrum/internal/globals"
...
func (wCtx *WebApiContext) IssueNewToken(respWriter http.ResponseWriter, request *http.Request) {
	...
	if len(realm) == 0 {
		...
		result = dto.ErrorDetails{Msg: errors_rest.RealmNotProviderMsg}
	} else {
		// todo: validate ...
		...
		if realmPtr == nil {
			...
			result = dto.ErrorDetails{Msg: stringFormatter.Format(errors_rest.RealmDoesNotExistsTemplate, realm)}
		} else {
			...
			if err != nil {
				...
				result = dto.ErrorDetails{Msg: errors_rest.BadBodyForTokenGenerationMsg}
			} else {
				...
				if err != nil {
					...
					if isRefresh == true {
						...
						if session == nil {
							...
							result = dto.ErrorDetails{Msg: errors_rest.InvalidTokenMsg, Description: errors_rest.TokenIsNotActive}
						} else {
	...
	if len(realm) == 0 {
		...
		if realmPtr == nil {
			...
			if parts[0] != string(BearerToken) {
				...
				status = http.StatusBadRequest
				result = dto.ErrorDetails{Msg: errors_rest.InvalidRequestMsg, Description: errors_rest.InvalidRequestDesc}
			} else {
				session := wCtx.Security.GetSessionByAccessToken(realm, &parts[1])
				if session == nil {
					wCtx.Logger.Debug("Get userinfo: invalid token")
					status = http.StatusUnauthorized
					result = dto.ErrorDetails{Msg: errors_rest.InvalidTokenMsg, Description: errors_rest.InvalidTokenDesc}
				} else {
					if session.Expired.Before(time.Now()) {
						...
						user, _ := wCtx.DataProvider.GetUserFromRealmById(realmPtr.Name, session.UserId)
						status = http.StatusOK
						if user != nil {
...
func (wCtx *WebApiContext) Introspect(respWriter http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		...
		result := dto.ErrorDetails{Msg: errors_rest.InvalidClientMsg, Description: errors_rest.InvalidClientCredentialDesc}
		afterHandle(&respWriter, status, &result)
		return
	}
